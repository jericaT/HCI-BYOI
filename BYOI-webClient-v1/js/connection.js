...
$(document).ready(function () {
    ...
    $('#imgCon').hover(function () {
        $('#imgCon').addClass('animated infinite pulse');
        $('#connectText').addClass('animated infinite pulse');
    });
    ...
    $('#imgCon').click(function () {
        ...
        BYOI.connection.onmessage = function (e) {
            ...
            if (type == 'PACKET') {
                text = received.text;
                from = received.from;
                $('#messageList').append("<id =\"receivedMsgs\" class=\"received\"><span class=\"node\"> " + from + "</span> : <span class=\"text\">" + text + "</span></div>");
            }
            if (type == 'TASK') {
                task = received.task;
